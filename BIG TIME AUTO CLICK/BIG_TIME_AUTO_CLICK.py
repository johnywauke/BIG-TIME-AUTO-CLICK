...
Telainicial = [773,591,774,592] #109,250,42
Telainicial2 = [867,390,868,391] #194,114,83
Telajogando = [785,71,786,72] #255,255,255
Telajogando2 = [794,78,795,79] #92,90,101
TelaNovamente = [883,168,884,169] #231 150 116
TelaNovamente2 = [784,699,785,700] #255 255 255

def quebrabloco():
 X = random.randrange(-50, 50)
 pyautogui.moveTo(753 + X, 363 + X)
 pyautogui.drag(0,-100, 0.2, button='left')

 pyautogui.moveTo(753 + X, 363 + X)
 pyautogui.drag(X, 0, 0.2, button='left')
...
while True:
   telajogo = np.array(ImageGrab.grab(bbox=Telajogando))
   telajogo2 = np.array(ImageGrab.grab(bbox=Telajogando2))
   color=np.array([255,255,255],dtype=np.uint8)
   color2=np.array([92,90,101],dtype=np.uint8)
   reds5=np.where(np.all((telajogo==color),axis=-1))
   reds6=np.where(np.all((telajogo2==color2),axis=-1))
   if len(reds5[0]) > 0 and len(reds6[0])> 0:
    start_time = time.time()
    quebrabloco()
    clicks_per_second =  (time.time() - start_time)
    print("Clicks per second {}".format(clicks_per_second))
   else:
     telajogo = np.array(ImageGrab.grab(bbox=TelaNovamente))
     telajogo2 = np.array(ImageGrab.grab(bbox=TelaNovamente2))
     color=np.array([231,150,116],dtype=np.uint8)
     ...
     if len(reds5[0]) > 0 and len(reds6[0])> 0:
      ...
      telajogo = np.array(ImageGrab.grab(bbox=Telainicial))
      telajogo2 = np.array(ImageGrab.grab(bbox=Telainicial2))
      color=np.array([109,250,42],dtype=np.uint8)
      color2=np.array([194,114,83],dtype=np.uint8)
      reds5=np.where(np.all((telajogo==color),axis=-1))
      reds6=np.where(np.all((telajogo2==color2),axis=-1))
      if len(reds5[0]) > 0 and len(reds6[0])> 0:
       start_time = time.time()
       ...
       keyboard.press_and_release('esc, esc')
       print('AD')
